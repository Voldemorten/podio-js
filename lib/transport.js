...
module.exports = {
    ...
    _handleTransportError: function (options, response) {
        ...
        switch (response.status) {
            case 400:
                return new PodioErrors.PodioBadRequestError(
                    response.body,
                    response.status,
                    request.url
                );
            case 401:
                ...
                return new PodioErrors.PodioAuthorizationError(
                    response.body,
                    response.status,
                    request.url
                );
            case 403:
                return new PodioErrors.PodioForbiddenError(
                    response.body,
                    response.status,
                    request.url
                );
            case 404:
                return new PodioErrors.PodioNotFoundError(
                    response.body,
                    response.status,
                    request.url
                );
            case 409:
                return new PodioErrors.PodioConflictError(
                    response.body,
                    response.status,
                    request.url
                );
            ...
            case 420:
                return new PodioErrors.PodioRateLimitError(
                    response.body,
                    response.status,
                    request.url
                );
            case 500:
                return new PodioErrors.PodioServerError(
                    response.body,
                    response.status,
                    request.url
                );
            ...
            case 503:
            case 504:
                return new PodioErrors.PodioUnavailableError(
                    response.body,
                    response.status,
                    request.url
                );
            ...
            if (isStatus401 && isInvalidOrExpiredToken && hasRefreshToken) {
                this._getAuth()._refreshToken(
                    function (err, response) {
                        ...
                        this._onTokenRefreshed(options.requestParams).then(
                            function (response) {
    ...
    request: function (method, path, data, options) {
        ...
        if ((!options || !options.basicAuth) && !this.authObject) {
            return new Promise(function (resolve, reject) {
                reject(
                    new PodioErrors.PodioForbiddenError('Authentication has not been performed')
                );
            });
        }
            ...
            return new Promise(function (resolve, reject) {
                reject(
                    new PodioErrors.PodioError(
                        'File uploads are only supported on the server right now.'
                    )
                );
            });
        }

        req = this._getRequestObject()
            .post(url)
